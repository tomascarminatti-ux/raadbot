...
def test_path_traversal_mitigated():
    ...
    response = client.post(
        "/api/v1/gems/refine",
        json={"gem_id": "../vuln_test", "instruction": "test"}
    )
...
def test_valid_gem_refine():
    ...
    try:
        ...
        print(f"Valid gem passed validation but failed later (expected): {type(e).__name__}")
        pass
...
def test_id_validation_pipeline():
    ...
    response = client.post(
        "/api/v1/run",
        json={"search_id": "invalid;id", "local_dir": "."}
    )
    assert response.status_code == 422
