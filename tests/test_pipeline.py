...
def test_pipeline_compiled_validator_correctness(mock_gemini, temp_output_dir):
    ...
    # Verify that validator is successfully compiled
    assert pipeline.validator is not None

    # Valid data according to schemas/gem_output.schema.json
    valid_data = {
        "meta": {
            "search_id": "SEARCH-2026-001",
            "gem": "GEM_1",
        ...
        "blockers": [],
        "content": {"key_insight": "Test insight"},
    }

    # Should validate successfully
    assert pipeline._validate_output(valid_data, "gem1") is True

    # Invalid data (missing required scores field)
    invalid_data = {
        "meta": {
            "search_id": "SEARCH-2026-001",
            "gem": "GEM_1",
    ...
    # Should raise ValueError due to missing 'scores'
    with pytest.raises(ValueError) as exc_info:
        pipeline._validate_output(invalid_data, "gem1")
    assert "Schema fallido" in str(exc_info.value)
