...
def test_pipeline_compiled_validator_correctness(mock_gemini, temp_output_dir):
    ...
    # 1. Verify validator is initialized and is compiled
    assert pipeline.schema is not None
    assert pipeline.validator is not None

    # 2. Test valid output validation
    valid_data = {
        "meta": {
            "search_id": "SEARCH-2026-001",
            "gem": "GEM_5",
    ...
    # 3. Test invalid output validation (missing required 'meta' field)
    invalid_data = {"scores": {"confidence": 9}, "blockers": [], "content": {}}
