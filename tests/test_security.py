...
def test_valid_requests():
    # We don't want to actually run the pipeline as it needs a real API key and structure
    # But we can check if it passes validation

    # Mocking the run_pipeline to avoid actual execution
    ...
    # If it passes validation, it might still fail later because runs/test doesn't exist,
    # but it won't be a 422 validation error.
    assert response.status_code != 422
