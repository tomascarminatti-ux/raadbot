...
def test_pipeline_request_valid_identifiers():
    req = PipelineRequest(search_id="search_123", candidate_id="cand-456", local_dir="data/inputs")
...
def test_pipeline_request_invalid_candidate_id():
    with pytest.raises(ValidationError) as excinfo:
        PipelineRequest(search_id="valid_search", candidate_id="../secret_candidate")
    assert "candidate_id" in str(excinfo.value)

def test_pipeline_request_invalid_local_dir_traversal():
    with pytest.raises(ValidationError) as excinfo:
        PipelineRequest(search_id="valid_search", local_dir="../relative/path")
...
def test_setup_search_request_validation():
    with pytest.raises(ValidationError) as excinfo:
        SetupSearchRequest(search_id="search/../id", brief_notes="notes", jd_content="jd")
    assert "search_id" in str(excinfo.value)
