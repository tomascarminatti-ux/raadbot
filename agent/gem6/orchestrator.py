...
class GEM6Orchestrator:
    ...
    async def run_pipeline(self, search_inputs: Dict[str, Any], candidates: Dict[str, Any]):
        ...
        results = {}
        semaphore = asyncio.Semaphore(5)  # Limit concurrency to 5 agents

        async def worker(candidate_id, candidate_data):
        ...
        tasks = [worker(cid, cdata) for cid, cdata in candidates.items()]
        pipeline_results = await asyncio.gather(*tasks)
        results = dict(pipeline_results)
    ...
    async def process_context(self, context_data: Dict[str, Any]):
        ...
        while step < max_steps:
            ...
            # 2. Call GEM 6 for reasoning (Offloaded to thread as it's blocking)
    ...
    async def call_agent(self, agent_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.gemini:
            try:
                ...
                # Offloaded to thread as it's blocking
