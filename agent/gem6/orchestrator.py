...
class GEM6Orchestrator:
    ...
    async def run_pipeline(self, search_inputs: Dict[str, Any], candidates: Dict[str, Any]):
        ...
        async def process_candidate_with_limit(candidate_id, candidate_data):
            context = {
        ...
        tasks = [process_candidate_with_limit(cid, cdata) for cid, cdata in candidates.items()]
        candidate_results = await asyncio.gather(*tasks)
        results = dict(candidate_results)
