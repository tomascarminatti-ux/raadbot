...
class GEM6Orchestrator:
    ...
    async def run_pipeline(self, search_inputs: Dict[str, Any], candidates: Dict[str, Any]):
        ...
        semaphore = asyncio.Semaphore(5)  # Limit concurrent candidates

        async def sem_process(candidate_id, candidate_data):
        ...
        tasks = [sem_process(cid, cdata) for cid, cdata in candidates.items()]
    ...
    async def process_context(self, context_data: Dict[str, Any]):
        ...
        while step < max_steps:
            ...
            # 2. Call GEM 6 for reasoning (offload sync LLM call to thread)
    ...
    async def call_agent(self, agent_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Calls the agent using GeminiClient (offloaded to thread) or fallback to mock"""
