...
class GEM6Orchestrator:
    ...
    async def run_pipeline(self, search_inputs: Dict[str, Any], candidates: Dict[str, Any]):
        ...
        Optimized: Processes candidates in parallel using asyncio.gather and a semaphore to control concurrency.
        """
        semaphore = asyncio.Semaphore(5)

        async def _bounded_process(candidate_id, candidate_data):
        ...
        tasks = [_bounded_process(cid, cdata) for cid, cdata in candidates.items()]
    ...
    async def process_context(self, context_data: Dict[str, Any]):
        ...
        while step < max_steps:
            ...
            # Optimized: Offload blocking LLM call to a worker thread
