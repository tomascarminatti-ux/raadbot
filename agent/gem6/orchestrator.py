...
class GEM6Orchestrator:
    ...
    async def run_pipeline(self, search_inputs: Dict[str, Any], candidates: Dict[str, Any]):
        ...
        async def process_one(candidate_id, candidate_data):
            context = {
            ...
            res = await self.process_context(context)
            return candidate_id, res

        # Process candidates concurrently to improve throughput
        tasks = [process_one(cid, cdata) for cid, cdata in candidates.items()]
    ...
    async def process_context(self, context_data: Dict[str, Any]):
        ...
        while step < max_steps:
            ...
            # 2. Call GEM 6 for reasoning - use to_thread to avoid blocking event loop
    ...
    async def call_agent(self, agent_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.gemini:
            try:
                ...
                # Use to_thread to avoid blocking event loop with synchronous run_gem call
