...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    """
    # Cargar prompt maestro y del GEM (cacheado vía load_prompt)
    ...
    if variables:
        # Pre-procesar variables para asegurar que sean strings (y handlear dicts)
        replacements = {}
        ...
        prompt = VARIABLE_PATTERN.sub(lambda m: replacements.get(m.group(1), m.group(0)), prompt)
