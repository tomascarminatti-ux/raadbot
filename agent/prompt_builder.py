...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Inyectar prompt maestro (puede contener más variables)
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # Inyectar variables en un solo paso usando regex
    ...
    # Manually handle VERSION if present in Maestro but not variables
    # (Maestro v1.2 has [VERSION] not {{VERSION}}, so it's fine)
...
def get_required_variables(gem_name: str) -> list[str]:
    ...
    """
    prompt = load_prompt(gem_name)
    # Also consider maestro variables
    maestro = load_maestro()
    full_prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    variables = VARIABLE_PATTERN.findall(full_prompt)
