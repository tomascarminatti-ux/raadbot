...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Preparar mapeo de reemplazo
    # Incluimos PROMPT_MAESTRO como una variable más para el reemplazo
    full_vars = variables.copy()
    full_vars["PROMPT_MAESTRO"] = maestro

    def replace_var(match):
        var_name = match.group(1)
        if var_name in full_vars:
            val = full_vars[var_name]
            if isinstance(val, dict):
                return json.dumps(val, ensure_ascii=False, indent=2)
            return str(val)
        return match.group(0)  # Devolver original si no está en variables

    # Inyectar variables en una sola pasada
