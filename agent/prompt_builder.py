...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    """
    # Cargar prompt maestro y del GEM (usan caché)
    maestro = load_maestro()
    prompt = load_prompt(gem_name)

    # Inyectar prompt maestro primero, ya que puede contener más variables
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # Preparar mapeo de variables (serializar dicts a JSON)
    # Se crea un nuevo diccionario para evitar modificar el original
    replacements = {}
    ...
    # Reemplazo en una sola pasada usando regex
