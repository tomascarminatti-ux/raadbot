...
# Pre-compile regex for variable replacement
...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Prepare variables for single-pass replacement
    def replace_var(match):
        key = match.group(1)
        if key == "VERSION":
            return match.group(0)  # Skip VERSION
        ...
        if key in variables:
            ...
            return str(value)
        return match.group(0)  # Keep original if key not in variables
