...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Inyectar prompt maestro (reemplazo simple)
    # Nota: El maestro también puede contener placeholders que serán resueltos en el siguiente paso.
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # Inyectar variables en una sola pasada usando regex y callback para máxima eficiencia
    ...
    prompt = VARIABLE_PATTERN.sub(_replace_match, prompt)
