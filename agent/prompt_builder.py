...
def load_maestro() -> str:
    """Carga el prompt maestro (con caché LRU)."""
...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    3. Reemplaza todas las {{variables}} en un solo paso optimizado
    ...
    # Pre-procesar variables para evitar formateos redundantes en el callback
    processed_vars = {}
    ...
    # Reemplazo en un solo paso mediante regex y diccionario de variables
    def replace_match(match):
        key = match.group(1)
        if key in processed_vars:
            return processed_vars[key]
        # Conservar el placeholder intacto si no está en variables (por ejemplo, {{VERSION}})
