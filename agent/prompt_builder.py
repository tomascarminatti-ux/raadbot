...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    4. Valida variables faltantes durante el mismo pase
    ...
    # Reemplazo en un solo pase usando regex pre-compilado
    prompt = VAR_PATTERN.sub(replace_var, prompt)

    if missing:
        print(f"  ⚠️  Variables sin reemplazar: {list(set(missing))}")
