...
def clear_prompt_caches():
    """Invalida las caches de prompts (útil después de una refinación)."""
...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Inyectar prompt maestro primero para permitir variables dentro de él (opcional pero usual)
    # o simplemente reemplazarlo. El comportamiento original era reemplazarlo en el prompt del GEM.
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # Preparar el diccionario de reemplazos con JSON strings si es necesario
    replacements = {}
    for k, v in variables.items():
        if isinstance(v, (dict, list)):
            replacements[k] = json.dumps(v, ensure_ascii=False, indent=2)
        else:
