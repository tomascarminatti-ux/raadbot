...
# Regex pre-compilado para la extracción de variables en los prompts
VAR_RE = re.compile(r"\{\{(\w+)\}\}")


@functools.lru_cache(maxsize=32)
def _load_prompt_cached(filepath: str, mtime: float) -> str:
    """Helper interno cacheado para leer prompts con invalidación automática por mtime."""
    with open(filepath, "r", encoding="utf-8") as f:
        return f.read()


def load_prompt(gem_name: str) -> str:
    """Carga un prompt desde el directorio de prompts utilizando cache LRU por mtime."""
