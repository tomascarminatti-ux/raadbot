...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Inyectar prompt maestro (se maneja como una variable más para
    # eficiencia en re.sub)
    all_vars = {**variables, "PROMPT_MAESTRO": maestro}

    # Inyectar variables en una sola pasada usando re.sub
    # Bolt Optimization: re.sub con callback es O(N) vs O(K*N)
    # de múltiples .replace()
