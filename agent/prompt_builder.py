...
# Pre-compiled regular expression to optimize template variable search
VAR_RE = re.compile(r"\{\{(\w+)\}\}")


@functools.lru_cache(maxsize=32)
def _load_prompt_cached(filepath: str, mtime: float) -> str:
    """Helper interno con LRU cache para cargar prompts evitando E/S repetida de disco."""
    with open(filepath, "r", encoding="utf-8") as f:
        return f.read()


def load_prompt(gem_name: str) -> str:
    """Carga un prompt desde el directorio de prompts usando LRU cache e invalidación por mtime."""
