...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Inyectar variables (single-pass regex substitution)
    processed_vars = {
        k: (json.dumps(v, ensure_ascii=False, indent=2) if isinstance(v, dict) else str(v))
        for k, v in variables.items()
    }

    prompt = re.sub(r"\{\{(\w+)\}\}", lambda m: processed_vars.get(m.group(1), m.group(0)), prompt)

    # Validar que no queden variables sin reemplazar
    remaining = re.findall(r"\{\{(\w+)\}\}", prompt)
    if remaining:
        # Filtrar VERSION y las que ya procesamos
        remaining = [v for v in remaining if v != "VERSION" and v not in processed_vars]
        if remaining:
