...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    4. Valida que no queden variables sin reemplazar (excepto VERSION)
    ...
    """
    # Cargar prompt maestro y del GEM (usan cache)
    maestro = load_maestro()
    prompt = load_prompt(gem_name)

    # Inyectar prompt maestro (pre-requisito para el reemplazo de variables general)
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # Definir callback para re.sub que maneja la inyección de variables
    ...
    # Inyectar variables en una sola pasada de regex
