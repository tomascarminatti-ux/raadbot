...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # ⚡ Optimización: Inyectar prompt maestro primero para permitir variables anidadas
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # ⚡ Optimización: Inyectar variables en una sola pasada usando regex (O(N) vs O(N*M))
    def replace_func(match):
        ...
        if key in variables:
            ...
            return str(val)
        return match.group(0)  # Dejar como está si no hay valor
