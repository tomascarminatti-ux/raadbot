...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    """
    # Cargar prompt maestro y del GEM (aprovechando el caché de load_prompt)
    maestro = load_maestro()
    prompt = load_prompt(gem_name)

    # Inyectar prompt maestro (puede contener placeholders que se resolverán en la siguiente fase)
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # Inyectar variables en una sola pasada usando regex sub con callback
    def _replace(match):
        key = match.group(1)
        if key in variables:
            val = variables[key]
    ...
    prompt = PLACEHOLDER_PATTERN.sub(_replace, prompt)

    # Validar que no queden variables sin reemplazar
    remaining = PLACEHOLDER_PATTERN.findall(prompt)
    if remaining:
