...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    """
    # Cargar prompt maestro y del GEM (Cacheados con @lru_cache)
    maestro = load_maestro()
    prompt = load_prompt(gem_name)

    # 1. Inyectar prompt maestro primero (puede contener placeholders)
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # 2. Inyectar variables en una sola pasada usando regex para mayor eficiencia
    def replace_match(match):
        ...
        if key in variables:
            ...
            return str(value)
        return match.group(0)  # Mantener {{variable}} si no está en el dict

    prompt = VARIABLE_PATTERN.sub(replace_match, prompt)

    # 3. Validar que no queden variables sin reemplazar
