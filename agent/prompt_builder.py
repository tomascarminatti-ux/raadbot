...
# Pre-compiled regex for matching {{variable}} placeholders in prompts
VAR_RE = re.compile(r"\{\{(\w+)\}\}")


@functools.lru_cache(maxsize=32)
def load_prompt(gem_name: str) -> str:
    """Carga un prompt desde el directorio de prompts (con caché LRU para evitar lecturas de disco repetidas)."""
...
def clear_prompt_caches() -> None:
    """Limpia la caché LRU de prompts cuando un prompt es modificado en disco."""
