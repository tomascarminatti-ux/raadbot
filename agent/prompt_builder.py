...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # El prompt maestro puede haber introducido sus propias variables o necesitamos conservarlas
    # Combinamos variables de entrada con el prompt_maestro (si fuera dinámico, pero aquí es estático)
    ...
    def replace_match(match):
        ...
        if key in variables:
            ...
            return str(value)
        return match.group(0)  # Mantener {{variable}} si no está en dict

    # Inyectar variables en un solo pase
    prompt = VARIABLE_PATTERN.sub(replace_match, prompt)

    # Validar que no queden variables sin reemplazar (ignorando VERSION)
    remaining = [m for m in VARIABLE_PATTERN.findall(prompt) if m != "VERSION"]
    if remaining:
