...
# Pre-compile regular expressions for template variable extraction
VAR_RE = re.compile(r"\{\{(\w+)\}\}")


@functools.lru_cache(maxsize=32)
def _load_prompt_cached(filepath: str, mtime: float) -> str:
    """Helper con LRU cache que recarga automáticamente si mtime cambia."""
    with open(filepath, "r", encoding="utf-8") as f:
        return f.read()


def load_prompt(gem_name: str) -> str:
    """Carga un prompt desde el directorio de prompts (cached por mtime)."""
