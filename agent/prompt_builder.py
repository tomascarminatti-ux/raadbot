...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Inyectar variables (segunda pasada única para eficiencia)
    ...
    prompt = VARIABLE_PATTERN.sub(replace_callback, prompt)

    # Validar que no queden variables sin reemplazar
    remaining_matches = VARIABLE_PATTERN.findall(prompt)
    if remaining_matches:
        # Filtrar VERSION que es metadata, no un input
        remaining = [v for v in remaining_matches if v != "VERSION"]
        if remaining:
