...
# Pre-compiled regex for variable placeholder extraction
VAR_RE = re.compile(r"\{\{(\w+)\}\}")


@functools.lru_cache(maxsize=32)
def _load_prompt_cached(filepath: str, mtime: float) -> str:
    """Helper cacheado para lectura de prompts con invalidación basada en mtime."""
    with open(filepath, "r", encoding="utf-8") as f:
        return f.read()


def load_prompt(gem_name: str) -> str:
    """Carga un prompt desde el directorio de prompts (con caché y auto-invalidación)."""
