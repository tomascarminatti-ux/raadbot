...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # 1. Inyectar prompt maestro (primera pasada para permitir variables dentro del maestro)
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    # 2. Inyectar el resto de las variables en un solo paso usando regex
    def replace_match(match):
        ...
        if key in variables:
            ...
            return str(val)
        return match.group(0)  # Mantener {{variable}} si no se encuentra
