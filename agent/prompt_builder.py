...
def build_prompt(gem_name: str, variables: dict) -> str:
    ...
    # Inyectar prompt maestro primero ya que puede contener mas variables
    prompt = prompt.replace("{{PROMPT_MAESTRO}}", maestro)

    def replace_match(match):
        key = match.group(1)
        if key == "PROMPT_MAESTRO":
            return maestro # Por si acaso quedara otro

        value = variables.get(key)
        if value is None:
            # Si no está en variables, lo dejamos como está para validación posterior
            # Excepto si es VERSION que es metadata
            if key == "VERSION":
...
def clear_caches():
    """Limpia las caches de prompts (útil para tests o si cambian los archivos)."""
