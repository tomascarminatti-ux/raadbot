...
class Pipeline:
    ...
    def _init_validator(self) -> Any:
        """Pre-compila el validador de JSON Schema para mejorar performance."""
        if not self.schema:
            return None
    ...
    def _validate_output(self, json_data: dict, gem_name: str) -> bool:
        """Valida el output JSON contra el schema pre-compilado."""
        if not self.validator or not json_data:
            raise ValueError(f"Output nulo o sin JSON válido en {gem_name}")
        try:
            # Performance boost: usando validador pre-compilado
