...
class Pipeline:
    ...
    def __init__(self, gemini: GeminiClient, search_id: str, output_dir: str):
        ...
        # Pre-compile jsonschema validator to avoid recompilation on every GEM validation step (~60x speedup)
        self.validator = (
            jsonschema.validators.validator_for(self.schema)(self.schema)
            if self.schema
            else None
        )
