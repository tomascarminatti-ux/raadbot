...
class Pipeline:
    ...
    def __init__(self, gemini: GeminiClient, search_id: str, output_dir: str):
        ...
        if self.schema:
            try:
                # Pre-compile the validator class and instance once to optimize validation performance (~13x speedup)
                validator_cls = validators.validator_for(self.schema)
                self.validator = validator_cls(self.schema)
            except Exception as e:
                logger.warning(f"⚠️ Error intentando precompilar validador JSON schema: {e}")
    ...
    def _validate_output(self, json_data: dict, gem_name: str) -> bool:
        ...
        try:
            # Reuse pre-compiled validator instance if available
            if self.validator:
