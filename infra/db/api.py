...
async def upsert_entity(data: EntityUpdate):
    ...
    try:
        ...
        if conn:
            conn.rollback()
        raise HTTPException(
            status_code=500, detail="Internal database error during upsert")
    finally:
        if conn:
            conn.close()
...
async def discard_entity(data: DiscardEntity):
    ...
    try:
        ...
        if conn:
            conn.rollback()
        raise HTTPException(
            status_code=500, detail="Internal database error during discard")
    finally:
        if conn:
            conn.close()
...
async def log_discovery(data: DiscoveryLog):
    ...
    try:
        ...
        if conn:
            conn.rollback()
        raise HTTPException(
            status_code=500, detail="Internal database error during logging")
    finally:
        if conn:
            conn.close()
