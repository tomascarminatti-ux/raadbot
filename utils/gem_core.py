...
class GEMClient:
    ...
    def _get_client(self) -> httpx.AsyncClient:
        """Lazy initialization of the persistent httpx.AsyncClient."""
    ...
    async def upsert_entity(self, data: Dict[str, Any]):
        """
        Optimization: Reuses the persistent HTTP session to reduce latency
        from TCP/TLS handshakes in successive calls.
        """
        try:
    ...
    async def discard_entity(self, data: Dict[str, Any]):
        """Reuses the persistent HTTP session."""
        try:
            client = self._get_client()
