...
class GEMClient:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        """Lazily initialize the httpx.AsyncClient for session reuse."""
        if self._client is None:
            # Optimization: Reusing a single AsyncClient significantly reduces overhead
            # by keeping TCP connections alive and avoiding repeated handshakes.
    ...
    async def upsert_entity(self, data: Dict[str, Any]):
        try:
            ...
            resp = await client.post(f"{self.db_url}/entity/upsert", json=data, timeout=30.0)
    ...
    async def discard_entity(self, data: Dict[str, Any]):
        try:
            client = await self._get_client()
            resp = await client.post(f"{self.db_url}/entity/discard", json=data, timeout=30.0)
            resp.raise_for_status()
            return resp.json()
        except Exception as e:
    ...
    async def log_execution(self, log_data: Dict[str, Any]):
        try:
            client = await self._get_client()
            resp = await client.post(f"{self.db_url}/log/discovery", json=log_data, timeout=30.0)
            resp.raise_for_status()
            return resp.json()
        except Exception as e:
