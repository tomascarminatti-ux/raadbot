...
**Action:** [Always add visual feedback and ARIA labels to new interactive elements. Avoid putting aria-labels on divs without roles. Also ensure utility or test scripts (like test_gemini.py) that use external services are wrapped in if __name__ == "__main__": to avoid collection failures in CI.]
