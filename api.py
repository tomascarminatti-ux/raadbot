...
class PipelineRequest(BaseModel):
    ...
    def validate_identifiers(cls, v: Optional[str]) -> Optional[str]:
        # Prevents path traversal by enforcing alphanumeric identifiers
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # Prevents directory traversal vectors in relative folder paths
        if v is not None:
            normalized = v.replace("\\", "/")
            if ".." in normalized or normalized.startswith("/") or ":" in normalized:
                raise ValueError("local_dir must not contain path traversal '..' or absolute path indicators.")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Prevents path traversal in output directory creation
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("search_id must contain only alphanumeric characters, dashes, and underscores.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Prevents path traversal when loading/modifying prompt template files
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("gem_id must contain only alphanumeric characters, dashes, and underscores.")
        return v
