...
async def run_pipeline(request: PipelineRequest) -> dict:
    ...
    # Defensa en profundidad: sanitizar IDs para evitar path traversal
    ...
    if request.local_dir:
        # Prevent absolute paths or directory traversal in local_dir
        if request.local_dir.startswith("/") or ".." in request.local_dir:
            raise ValueError("Directorio local inválido.")
