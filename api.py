...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Security concern: prevent path traversal or unexpected input patterns in run dir
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "search_id must be alphanumeric with dashes or underscores only"
            )
        return v
    ...
    def validate_candidate_id(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return v
        # Security concern: prevent directory traversal or unexpected input patterns in candidates
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "candidate_id must be alphanumeric with dashes or underscores only"
            )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return v
        # Security concern: prevent absolute paths, directory traversal and normalize separators
        v_normalized = v.replace("\\", "/")
        if ".." in v_normalized or v_normalized.startswith("/") or ":" in v_normalized:
            raise ValueError(
                "local_dir must be relative and cannot contain directory traversal '..' sequences"
            )
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    @classmethod
    def validate_search_id(cls, v: str) -> str:
        # Security concern: prevent path traversal or unexpected input patterns in run setup
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "search_id must be alphanumeric with dashes or underscores only"
            )
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security concern: prevent arbitrary file modification via traversal or unexpected IDs
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "gem_id must be alphanumeric with dashes or underscores only"
            )
        # Ensure it matches existing GEM prompts only to avoid reading/writing arbitrary files
        if v not in ["gem1", "gem2", "gem3", "gem4", "gem5"]:
            raise ValueError("Invalid gem_id")
        return v
