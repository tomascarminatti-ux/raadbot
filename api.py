...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("search_id contains invalid characters. Only alphanumeric, dashes and underscores are allowed.")
        return v
    ...
    def validate_candidate_id(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("candidate_id contains invalid characters. Only alphanumeric, dashes and underscores are allowed.")
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            # Block directory traversal sequences and absolute/root paths to prevent arbitrary file access
            if ".." in v or v.startswith("/") or v.startswith("\\") or ":" in v:
                raise ValueError("local_dir is invalid or contains path traversal sequences.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if v not in {"gem1", "gem2", "gem3", "gem4", "gem5"}:
            raise ValueError("gem_id must be a valid gem ID (gem1 - gem5).")
        return v
