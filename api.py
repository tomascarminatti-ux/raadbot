...
class PipelineRequest(BaseModel):
    ...
    # Sentinel Security Guard: Prevent directory traversal & injection on paths / IDs
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if (
                ...
                raise ValueError(
                    "local_dir contains invalid path traversal or absolute path characters."
                )
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    # Sentinel Security Guard: Prevent directory traversal via search_id
...
class RefineRequest(BaseModel):
    ...
    # Sentinel Security Guard: Prevent file path manipulation via gem_id
