...
class PipelineRequest(BaseModel):
    ...
    candidate_id: Optional[str] = Field(None, pattern='^[a-zA-Z0-9_-]+$', max_length=100)
    ...
    def prevent_path_traversal(cls, v: Optional[str]) -> Optional[str]:
        if v:
            if ".." in v or v.startswith("/") or ":" in v:
                raise ValueError("Path traversal or absolute path detected in local_dir")
        return v
...
class RefineRequest(BaseModel):
    gem_id: str = Field(..., pattern='^[a-zA-Z0-9_-]+$', max_length=20)
    instruction: str
