...
class PipelineRequest(BaseModel):
    # Regex validation prevents path traversal by restricting characters
    search_id: str = Field(pattern=config.ID_PATTERN)
    drive_folder: Optional[str] = None
    local_dir: Optional[str] = Field(default=None, pattern=config.ID_PATTERN)
    candidate_id: Optional[str] = Field(default=None, pattern=config.ID_PATTERN)
