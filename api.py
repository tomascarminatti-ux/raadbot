...
async def refine_gem(request: RefineRequest):
    ...
    # Security check: Ensure gem_id is whitelisted to prevent path traversal and unauthorized file access
    if request.gem_id not in config.ALLOWED_GEMS:
        raise HTTPException(status_code=403, detail="Access to this GEM is not allowed")
