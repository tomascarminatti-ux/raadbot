...
class PipelineRequest(BaseModel):
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal / command injection via identifiers
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if ".." in normalized or normalized.startswith("/") or (len(normalized) > 1 and normalized[1] == ":"):
                raise ValueError("Invalid local_dir: directory traversal or absolute path detected.")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Security: Enforce safe alphanumeric identifier
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Validate GEM identifier to avoid path traversal when loading prompt files
        if not re.fullmatch(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("Identifier must contain only alphanumeric characters, underscores, and hyphens.")
        return v
