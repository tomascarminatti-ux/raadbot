...
class PipelineRequest(BaseModel):
    ...
    def validate_identifiers(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal by ensuring identifier is strict alphanumeric with dashes/underscores
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal by rejecting parent directory references or absolute paths
        if v is not None:
            normalized = v.replace("\\", "/")
            if ".." in normalized or normalized.startswith("/") or re.match(r"^[a-zA-Z]:", normalized):
                raise ValueError("Path traversal vectors or absolute paths are prohibited.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal by validating gem_id against allowed pattern
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("Identifier must contain only alphanumeric characters, dashes, or underscores.")
        return v
