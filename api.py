...
class PipelineRequest(BaseModel):
    ...
    def validate_ids(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("ID contains invalid characters or path traversal sequences")
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if ".." in normalized or normalized.startswith("/") or ":" in normalized:
                raise ValueError("local_dir contains path traversal sequences or absolute path")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("search_id contains invalid characters or path traversal sequences")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("gem_id contains invalid characters or path traversal sequences")
        return v
