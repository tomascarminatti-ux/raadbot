...
class PipelineRequest(BaseModel):
    ...
    candidate_id: Optional[str] = Field(default=None, pattern=config.ID_PATTERN)  # Si se quiere procesar solo uno
...
async def refine_gem(request: RefineRequest):
    ...
    # Whitelist check for allowed GEMs
    if request.gem_id not in config.ALLOWED_GEMS:
        raise HTTPException(status_code=403, detail="Access to this GEM is restricted or it does not exist.")
