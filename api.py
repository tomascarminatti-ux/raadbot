...
class PipelineRequest(BaseModel):
    ...
    def validate_identifiers(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "Identifier must contain only alphanumeric characters, dashes, or underscores."
            )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if (
                ".." in v_norm
                or v_norm.startswith("/")
                or re.match(r"^[a-zA-Z]:", v_norm)
            ):
                raise ValueError(
                    "Path traversal sequences are not allowed in local_dir."
                )
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Sentinel Security: Prevent path traversal in directory creation (e.g. runs/<search_id>/outputs)
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Sentinel Security: Prevent path traversal when loading/writing prompt files (e.g. prompts/<gem_id>.md)
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
