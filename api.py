...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal attacks when referencing prompt files on disk.
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "Invalid gem_id format. Only alphanumeric characters, dashes, and underscores are allowed."
            )
        return v
