...
class PipelineRequest(BaseModel):
    ...
    def validate_identifiers(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("ID contains invalid characters (possible path traversal).")
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if ".." in normalized or normalized.startswith("/") or re.match(r"^[a-zA-Z]:", normalized):
                raise ValueError("Invalid directory path (possible path traversal).")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("search_id contains invalid characters (possible path traversal).")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("gem_id contains invalid characters (possible path traversal).")
        return v
