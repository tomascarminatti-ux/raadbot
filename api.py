...
def validate_identifier(v: Optional[str]) -> Optional[str]:
    ...
    if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
        raise ValueError(
            "Identifier must contain only alphanumeric characters, dashes, and underscores."
        )
    return v
...
def validate_local_dir(v: Optional[str]) -> Optional[str]:
    ...
    if (
        ...
        raise ValueError(
            "Invalid directory path: Directory traversal, absolute path, or drive letter detected."
        )
    return v
