...
class PipelineRequest(BaseModel):
    ...
    def validate_identifiers(cls, v: Optional[str]) -> Optional[str]:
        # SECURITY: Prevent directory traversal (e.g., ../) and unauthorized path manipulation
        # by restricting identifiers to alphanumeric, underscores, and hyphens only.
        if v is not None:
            if not re.match(r"^[a-zA-Z0-9_-]+$", v):
                raise ValueError(
                    "Identifiers must be alphanumeric and contain only letters, numbers, underscores, or hyphens."
                )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # SECURITY: Restrict local directory path input to prevent directory traversal
        # and unauthorized file access across the system filesystem.
        if v is not None:
            # Normalize backslashes to forward slashes
            normalized = v.replace("\\", "/")
            # Block directory traversal sequences completely
            if ".." in normalized.split("/"):
                raise ValueError("Directory traversal sequence ('..') is not allowed.")
            # Enforce that paths are relative and do not escape workspace root
            if (
                ...
                raise ValueError(
                    "local_dir must be a relative subdirectory under the current path and not start with / or ./ or ../"
                )
            # Disallow absolute paths, drive letters, and unc root paths
            if ":" in normalized or normalized.startswith("//"):
                raise ValueError(
                    "Absolute paths or paths with drive letters are not allowed."
                )
            return normalized
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # SECURITY: Prevent directory traversal and path manipulation by restricting identifiers to alphanumeric, underscores, and hyphens.
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "search_id must be alphanumeric and contain only letters, numbers, underscores, or hyphens."
            )
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # SECURITY: Prevent directory traversal and prompt override attacks by restricting gem_id to alphanumeric, underscores, and hyphens.
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "gem_id must be alphanumeric and contain only letters, numbers, underscores, or hyphens."
            )
        return v
