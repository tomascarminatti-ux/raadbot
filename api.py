...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not v or not v.strip():
            raise ValueError("search_id cannot be empty or whitespace")
        if ".." in v or "/" in v or "\\" in v:
            raise ValueError(
                "Path traversal characters or separators are not allowed in search_id"
            )
        if not re.match(r"^[a-zA-Z0-9_\-]+$", v):
            raise ValueError(
                "search_id contains invalid characters. Only alphanumeric, hyphens, and underscores are allowed."
            )
        return v
    ...
    def validate_candidate_id(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return v
        if not v.strip():
            raise ValueError("candidate_id cannot be empty or whitespace")
        if ".." in v or "/" in v or "\\" in v:
            raise ValueError(
                "Path traversal characters or separators are not allowed in candidate_id"
            )
        if not re.match(r"^[a-zA-Z0-9_\-]+$", v):
            raise ValueError(
                "candidate_id contains invalid characters. Only alphanumeric, hyphens, and underscores are allowed."
            )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        ...
        if ".." in normalized:
            raise ValueError(
                "Directory traversal sequence '..' is not allowed in local_dir"
            )
        if normalized.startswith("/") or re.match(r"^[a-zA-Z]:", normalized):
            raise ValueError(
                "Absolute paths are not allowed in local_dir for security reasons"
            )
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not v or not v.strip():
            raise ValueError("gem_id cannot be empty")
        allowed_gems = {"gem1", "gem2", "gem3", "gem4", "gem5"}
        if v not in allowed_gems:
            raise ValueError(f"gem_id must be one of {allowed_gems}")
        return v
