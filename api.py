...
class PipelineRequest(BaseModel):
    # Validar que search_id sea un identificador seguro (alfanumérico, guiones)
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        """Evita path traversal y acceso a directorios absolutos."""
        if v is None:
            return v
        # Bloquear path traversal y rutas absolutas por seguridad
        if ".." in v or v.startswith("/") or (len(v) > 1 and v[1] == ":"):
            raise ValueError("Ruta local_dir no permitida por seguridad.")
        return v
...
class RefineRequest(BaseModel):
    # Validar gem_id para evitar inyección de rutas en la carga de prompts
