...
class PipelineRequest(BaseModel):
    ...
    def validate_ids(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            if not re.match(r"^[a-zA-Z0-9_-]+$", v):
                raise ValueError(
                    "Identifier must only contain alphanumeric characters, underscores, or hyphens"
                )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if ".." in parts or normalized.startswith("/") or os.path.isabs(normalized):
                raise ValueError(
                    "Directory traversal or absolute paths are not allowed"
                )
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "GEM ID must only contain alphanumeric characters, underscores, or hyphens"
            )
        return v
