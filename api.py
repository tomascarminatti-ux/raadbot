...
class PipelineRequest(BaseModel):
    ...
    # Security: Validate search_id and candidate_id to avoid path traversal / injection
    @field_validator("search_id", "candidate_id")
    @classmethod
    def validate_safe_id(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
                raise ValueError("ID contains invalid characters. Only alphanumeric, dashes, and underscores allowed.")
        return v

    # Security: Validate local_dir to prevent directory traversal
    @field_validator("local_dir")
    @classmethod
    def validate_safe_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            normalized = v.replace("\\", "/")
            parts = [p for p in normalized.split("/") if p]
            if ".." in parts or normalized.startswith("/") or (len(normalized) > 1 and normalized[1] == ":"):
                raise ValueError("Directory traversal or absolute path detected in local_dir.")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    # Security: Validate search_id to avoid path traversal / injection
    @field_validator("search_id")
    @classmethod
    def validate_search_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("search_id contains invalid characters. Only alphanumeric, dashes, and underscores allowed.")
        return v
...
class RefineRequest(BaseModel):
    ...
    # Security: Validate gem_id to prevent arbitrary file reading/writing via path traversal
    @field_validator("gem_id")
    @classmethod
    def validate_gem_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("gem_id contains invalid characters. Only alphanumeric, dashes, and underscores allowed.")
        return v
