...
class PipelineRequest(BaseModel):
    ...
    # Sentinel Security: Prevenir Path Traversal en identificadores y rutas
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "Identificador inválido: solo se permiten caracteres alfanuméricos, '-' y '_'."
            )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if ".." in normalized.split("/") or os.path.isabs(v):
                raise ValueError(
                    "Ruta inválida: no se permite navegación de directorios (path traversal)."
                )
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    # Sentinel Security: Prevenir Path Traversal en search_id
    @field_validator("search_id")
    @classmethod
    def validate_search_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "search_id inválido: solo se permiten caracteres alfanuméricos, '-' y '_'."
            )
        return v
...
class RefineRequest(BaseModel):
    ...
    # Sentinel Security: Prevenir Path Traversal en gem_id
    @field_validator("gem_id")
    @classmethod
    def validate_gem_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "gem_id inválido: solo se permiten caracteres alfanuméricos, '-' y '_'."
            )
        return v
