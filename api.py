...
class PipelineRequest(BaseModel):
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal by restricting identifiers to strict alphanumeric/hyphen/underscore
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "Invalid identifier format: must contain only alphanumeric, hyphen, or underscore."
            )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # Security: Protect local folder paths from relative traversal or absolute path injection
        if v is not None:
            ...
            if (
                ...
                raise ValueError(
                    "Path traversal or absolute path detected in local_dir."
                )
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Security: Prevent path traversal by restricting search_id format
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "Invalid search_id format: must contain only alphanumeric, hyphen, or underscore."
            )
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal by restricting gem_id format
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "Invalid gem_id format: must contain only alphanumeric, hyphen, or underscore."
            )
        return v
