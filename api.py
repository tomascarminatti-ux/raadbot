...
class PipelineRequest(BaseModel):
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            # Security: Prevent path traversal by restricting identifiers to alphanumeric, underscores, and dashes
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            # Security: Prevent path traversal and arbitrary filesystem access in local_dir
            normalized = v.replace("\\", "/")
            if ".." in normalized.split("/") or normalized.startswith("/") or re.match(r"^[a-zA-Z]:", normalized):
                raise ValueError("Invalid local_dir: relative traversal and absolute paths not allowed.")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            # Security: Prevent path traversal by restricting search_id to alphanumeric, underscores, and dashes
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            # Security: Prevent path traversal by restricting gem_id to alphanumeric, underscores, and dashes
