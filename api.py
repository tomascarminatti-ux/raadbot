...
class PipelineRequest(BaseModel):
    ...
    @field_validator("drive_folder", "local_dir")
    @classmethod
    def validate_path(cls, v: Optional[str]) -> Optional[str]:
        ...
        if ".." in v or v.startswith("/") or v.startswith("~"):
            raise ValueError(
                "Invalid path: directory traversal or absolute paths not allowed")
        return v
...
async def trigger_pipeline(request: PipelineRequest, background_tasks: BackgroundTasks):
    ...
    if request.webhook_url:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Pipeline execution failed")
...
async def refine_gem(request: RefineRequest):
    ...
    if new_prompt:
        try:
            ...
            return {"status": "error", "message": "Failed to save refined prompt"}
