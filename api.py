...
class PipelineRequest(BaseModel):
    ...
    def validate_identifiers(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal by validating identifiers against alphanumeric chars, dashes, and underscores
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # Security: Sanitize local_dir to prevent directory traversal
        if v is not None:
            normalized = v.replace("\\", "/")
            if ".." in normalized.split("/") or os.path.isabs(v):
                raise ValueError("Ruta de directorio no permitida: no se permiten rutas absolutas ni secuencias de navegación '..'.")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Security: Ensure search_id contains no path traversal sequences
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal on prompt file paths (e.g. prompts/{gem_id}.md)
        if not ID_REGEX.match(v):
