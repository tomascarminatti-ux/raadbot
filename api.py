...
class PipelineRequest(BaseModel):
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            # Security: Prevent path traversal in search_id and candidate_id
            raise ValueError("Identifier must contain only letters, numbers, underscores, or hyphens.")
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            # Security: Prevent path traversal or absolute paths in local_dir
            normalized = v.replace("\\", "/")
            if ".." in normalized.split("/") or normalized.startswith("/") or ":" in normalized:
                raise ValueError("Invalid local_dir: directory traversal and absolute paths are forbidden.")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            # Security: Prevent path traversal in search_id
            raise ValueError("search_id must contain only letters, numbers, underscores, or hyphens.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.fullmatch(r"gem[1-5]", v):
            # Security: Restrict gem_id to valid gem names (gem1 to gem5) to prevent file traversal
            raise ValueError("gem_id must be a valid GEM identifier (gem1 through gem5).")
        return v
