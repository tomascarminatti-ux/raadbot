...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "search_id must only contain alphanumeric characters, hyphens, and underscores."
            )
        return v

    @field_validator("candidate_id")
    @classmethod
    def validate_candidate_id(cls, v: str | None) -> str | None:
        if v is not None and not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "candidate_id must only contain alphanumeric characters, hyphens, and underscores."
            )
        return v
    ...
    def validate_local_dir(cls, v: str | None) -> str | None:
        if v is not None:
            # Normalize all backslashes to forward slashes for unified cross-platform check
            v_unified = v.replace("\\", "/")
            normalized = os.path.normpath(v_unified)
            parts = v_unified.split("/")
            ...
            if (
                ...
                or os.path.isabs(v)
                or v.startswith(("/", "\\"))
                or v_unified.startswith("/")
                or ":" in v
            ):
                raise ValueError(
                    "local_dir must be a relative subdirectory and cannot contain path traversal or absolute/drive paths."
                )
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "gem_id must only contain alphanumeric characters, hyphens, and underscores."
            )
        return v
