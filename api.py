...
class PipelineRequest(BaseModel):
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v:
            if os.path.isabs(v):
                raise ValueError("Absolute paths are not allowed for local_dir")
            if ".." in v:
                raise ValueError("Parent directory references (..) are not allowed in local_dir")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.match(ID_PATTERN, v):
            raise ValueError(f"search_id contains invalid characters: {v}")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.match(ID_PATTERN, v):
            raise ValueError(f"gem_id contains invalid characters: {v}")
        return v
