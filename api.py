...
async def execute_orchestration(request: PipelineRequest, api_key: str):
    """Internal helper to setup and run orchestrator with proper cleanup"""
...
async def run_pipeline(request: PipelineRequest) -> dict:
    ...
    output_dir = await execute_orchestration(request, api_key)
...
async def setup_search(request: SetupSearchRequest):
    ...
    try:
        ...
        with open(os.path.join(output_dir, "gem5.json"), "w", encoding="utf-8") as f:
            json.dump(result.get("json", {}), f, indent=4)
        ...
        return {
            "status": "success",
            "search_id": request.search_id,
            "gem5_summary": result.get("json", {}).get("mandate_summary", "Mandato generado con éxito.")
        }
    finally:
        await gemini.close()
