...
class PipelineRequest(BaseModel):
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("Identificador inválido: solo caracteres alfanuméricos, guiones y guiones bajos son permitidos.")
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if ".." in normalized or normalized.startswith("/") or re.match(r"^[a-zA-Z]:", normalized):
                raise ValueError("Acceso a directorio inválido: secuencias de navegación o rutas absolutas no permitidas.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError("Identificador de GEM inválido.")
        return v
