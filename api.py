...
class PipelineRequest(BaseModel):
    ...
    candidate_id: Optional[str] = Field(None, pattern=ID_PATTERN)  # Si se quiere procesar solo uno
...
class RefineRequest(BaseModel):
    gem_id: str = Field(pattern=ID_PATTERN)
    instruction: str
...
async def refine_gem(request: RefineRequest):
    ...
    # Defense in depth: resolve path and verify prefix
    ...
    if not prompt_path.startswith(base_dir):
        raise HTTPException(status_code=422, detail="Invalid GEM ID")
