...
class PipelineRequest(BaseModel):
    # Regex to allow only alphanumeric, underscores and hyphens to prevent path traversal/injection
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v:
            # Prevent path traversal by rejecting ".." and absolute paths
            if ".." in v or v.startswith("/") or (len(v) > 1 and v[1] == ":"):
                raise ValueError("local_dir contains invalid path sequences or is absolute")
        return v
