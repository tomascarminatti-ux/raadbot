...
async def trigger_pipeline(request: PipelineRequest, background_tasks: BackgroundTasks):
    ...
    if request.webhook_url:
        ...
        try:
            ...
            print(f"Error during pipeline execution for search_id={request.search_id}: {e}")
            raise HTTPException(
                status_code=500,
...
async def refine_gem(request: RefineRequest):
    ...
    # Security: Whitelist GEM IDs to prevent path traversal
    if request.gem_id not in config.ALLOWED_GEMS:
        raise HTTPException(status_code=403, detail="Access to this GEM is restricted")
