...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError(
                "search_id must be alphanumeric, dashes, or underscores only"
            )
        return v
    ...
    def validate_candidate_id(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            if not re.match(r"^[a-zA-Z0-9_-]+$", v):
                raise ValueError(
                    "candidate_id must be alphanumeric, dashes, or underscores only"
                )
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("gem_id must be alphanumeric, dashes, or underscores only")
        return v
