...
class PipelineRequest(BaseModel):
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal and special character injection in identifiers
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # Security: Normalize separators and prevent directory traversal via '..' path segments
        if v is not None:
            normalized = v.replace("\\", "/")
            if ".." in normalized.split("/"):
                raise ValueError("Directory traversal ('..') is strictly prohibited.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal in gem_id
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
