...
class PipelineRequest(BaseModel):
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if ".." in normalized or normalized.startswith("/") or re.match(r"^[a-zA-Z]:", v):
                raise ValueError("Invalid directory path: path traversal detected.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("gem_id must contain only letters, numbers, underscores, or hyphens.")
        return v
