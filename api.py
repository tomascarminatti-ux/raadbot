    ...
    def prevent_traversal(cls, v):
        if v:
            if ".." in v or os.path.isabs(v):
                raise ValueError(
                    "local_dir cannot contain '..' or be an absolute path")
        return v
...
async def setup_search(request: SetupSearchRequest):
    ...
    return {
        "status": "success",
        "search_id": request.search_id,
        "gem5_summary": result.get(
            "data",
            {}).get(
            "mandate_summary",
            "Mandato generado con éxito.")}
...
class RefineRequest(BaseModel):
    gem_id: str = Field(pattern=r"^gem[0-9]+$")
    instruction: str
...
async def refine_gem(request: RefineRequest):
    ...
    if not os.path.exists(prompt_path):
        raise HTTPException(
            status_code=404,
            detail="GEM prompt file not found")
