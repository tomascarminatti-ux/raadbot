...
def _validate_id(v: Optional[str], field_name: str) -> Optional[str]:
    # Security: Validate identifier fields to prevent path traversal and arbitrary file access
    if v is not None and not ID_REGEX.match(v):
        raise ValueError(
            f"{field_name} must contain only alphanumeric characters, underscores, or hyphens."
        )
    return v
...
class PipelineRequest(BaseModel):
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal in local_dir inputs
        if v is not None:
            normalized = v.replace("\\", "/")
            if ".." in normalized.split("/") or os.path.isabs(v):
                raise ValueError(
                    "local_dir must not contain directory traversal or absolute path."
                )
        return v
