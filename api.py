...
class PipelineRequest(BaseModel):
    ...
    def validate_identifier(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal by validating identifiers against alphanumeric patterns
        if v is not None and not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "Identifier must contain only alphanumeric characters, underscores, or dashes"
            )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal when loading local directories
        if v is not None:
            normalized = v.replace("\\", "/")
            if ".." in normalized.split("/"):
                raise ValueError(
                    "local_dir cannot contain path traversal sequences ('..')"
                )
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Security: Prevent path traversal when creating search output directories
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "search_id must contain only alphanumeric characters, underscores, or dashes"
            )
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal when reading/writing prompt files
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "gem_id must contain only alphanumeric characters, underscores, or dashes"
            )
        return v
