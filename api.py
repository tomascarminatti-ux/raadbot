...
# --- Regex para validar identificadores seguros contra path traversal ---
...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        if not IDENTIFIER_REGEX.match(v):
            raise ValueError("search_id must only contain alphanumeric characters, underscores, or hyphens.")
        return v

    @field_validator("candidate_id")
    @classmethod
    def validate_candidate_id(cls, v: Optional[str]) -> Optional[str]:
        if v is not None and not IDENTIFIER_REGEX.match(v):
            raise ValueError("candidate_id must only contain alphanumeric characters, underscores, or hyphens.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        if not IDENTIFIER_REGEX.match(v):
            raise ValueError("gem_id must only contain alphanumeric characters, underscores, or hyphens.")
        return v
