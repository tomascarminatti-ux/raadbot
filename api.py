...
def _validate_identifier(v: Optional[str]) -> Optional[str]:
    """Validates alphanumeric identifiers to prevent path traversal attacks."""
    if v is not None:
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", v):
            raise ValueError(
                "Invalid identifier. Must contain only alphanumeric characters, dashes, and underscores."
            )
    return v


def _validate_relative_dir(v: Optional[str]) -> Optional[str]:
    """Validates relative directory paths to prevent path traversal vectors."""
    if v is not None:
        normalized = v.replace("\\", "/")
        if ".." in normalized or normalized.startswith("/") or re.match(r"^[a-zA-Z]:", normalized):
            raise ValueError("Invalid local_dir: path traversal sequences or absolute paths are prohibited.")
    return v
...
class PipelineRequest(BaseModel):
    ...
    def validate_dir(cls, v):
        return _validate_relative_dir(v)
