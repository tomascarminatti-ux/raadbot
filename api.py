...
class PipelineRequest(BaseModel):
    # Validation to prevent path traversal and restrict characters
    search_id: str = Field(pattern=r'^[a-zA-Z0-9_-]+$')
    drive_folder: Optional[str] = None
    local_dir: Optional[str] = Field(None, pattern=r'^[a-zA-Z0-9._-][a-zA-Z0-9._/-]*$')
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v and ".." in v:
            raise ValueError("Path traversal ('..') not allowed in local_dir")
        return v
