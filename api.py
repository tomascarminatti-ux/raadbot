...
async def run_pipeline(request: PipelineRequest) -> dict:
    ...
    search_id = os.path.basename(request.search_id)
    output_dir = os.path.join("runs", search_id, "outputs")
    os.makedirs(output_dir, exist_ok=True)

    gemini = GeminiClient(api_key=api_key, model=request.model)
    orchestrator = GEM6Orchestrator(gemini=gemini, search_id=search_id, output_dir=output_dir)
...
async def refine_gem(request: RefineRequest):
    ...
    gem_id = os.path.basename(request.gem_id)
    prompt_path = f"prompts/{gem_id}.md"
