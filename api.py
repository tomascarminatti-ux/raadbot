...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Security: Prevent path traversal (CWE-22) in search_id to secure file operations in 'runs/<search_id>/outputs'
        if ".." in v or "/" in v or "\\" in v:
            raise ValueError(
                "Path traversal characters (.., /, \\) are not allowed in search_id."
            )
        return v
    ...
    def validate_candidate_id(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return v
        # Security: Prevent path traversal (CWE-22) in candidate_id
        if ".." in v or "/" in v or "\\" in v:
            raise ValueError(
                "Path traversal characters (.., /, \\) are not allowed in candidate_id."
            )
        return v
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return v
        # Security: Normalize backslashes to forward slashes for robust cross-platform path safety checks
        normalized = v.replace("\\", "/")
        # Prevent directory traversal attempts (CWE-22)
        if ".." in normalized:
            raise ValueError("Directory traversal ('..') is not allowed in local_dir.")
        # Reject absolute paths (starting with / or a Windows drive letter) to contain searches to relative project directories
        if normalized.startswith("/"):
            raise ValueError(
                "Absolute paths starting with '/' are not allowed in local_dir."
            )
        import re

        if re.match(r"^[a-zA-Z]:", normalized):
            raise ValueError(
                "Absolute paths with drive letters are not allowed in local_dir."
            )
        return normalized
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal (CWE-22) in gem_id to prevent reading/writing arbitrary files via prompt_path
        if ".." in v or "/" in v or "\\" in v:
            raise ValueError(
                "Path traversal characters (.., /, \\) are not allowed in gem_id."
            )
        return v
