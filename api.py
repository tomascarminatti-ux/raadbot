...
class PipelineRequest(BaseModel):
    ...
    def validate_safe_id(cls, v: Optional[str]) -> Optional[str]:
        # Security: Prevent path traversal by restricting identifiers to strict alphanumeric format
    ...
    def validate_local_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            # Security: Normalize separators and prevent relative/absolute path traversal attacks
            normalized = v.replace("\\", "/")
            if ".." in normalized or normalized.startswith("/") or re.match(r"^[a-zA-Z]:", normalized):
                raise ValueError("Path traversal attempt detected in local_dir.")
        return v
...
class SetupSearchRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Security: Prevent path traversal in search setup directory paths
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Security: Prevent path traversal in prompt file loading/writing
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
