...
class PipelineRequest(BaseModel):
    ...
    def validate_search_id(cls, v: str) -> str:
        # Enforce alphanumeric, underscore, or hyphen to prevent path traversal
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("search_id can only contain alphanumeric characters, underscores, and hyphens.")
        return v
    ...
    def validate_candidate_id(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None and not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("candidate_id can only contain alphanumeric characters, underscores, and hyphens.")
        return v
...
class RefineRequest(BaseModel):
    ...
    def validate_gem_id(cls, v: str) -> str:
        # Enforce alphanumeric, underscore, or hyphen to prevent path traversal
        if not re.match(r"^[a-zA-Z0-9_-]+$", v):
            raise ValueError("gem_id can only contain alphanumeric characters, underscores, and hyphens.")
        return v
